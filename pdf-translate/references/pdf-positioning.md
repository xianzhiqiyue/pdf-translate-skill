...
- Fonts and exact typography: built-in Helvetica is portable for English/Latin output, but many fonts do not cover Chinese/Japanese/Korean or other non-Latin scripts. Use `recommended_args.font_scripts` and `recommended_args.font_install_hint` from extraction JSON as the font preflight. Install a supporting font if needed; the script auto-selects a matching bundled/system font for non-Latin translations and fails before writing if none is found, preventing `?` output. Bundled fonts can be placed in `assets/fonts/` and are searched before system fonts. You can also use `--fontfile auto` or an explicit `.ttf/.otf/.ttc` path. CJK should use a real CJK font such as Noto Sans CJK or Source Han Sans. Arabic/Devanagari/Thai/Hebrew and other complex scripts need visual inspection because glyph coverage and shaping are separate concerns. The script preserves position and color, not exact original font identity.
- Multi-span labels split into several PDF drawing commands: the script groups by text line by default. If this creates awkward translations, use `--extract-only-json`, let the invoking assistant edit segment translations, then re-apply with `--translations-json`. For true multi-line paragraphs where uniform font size matters, use `--fit-scope block`; it inserts one union textbox per extracted text block, at the cost of less exact per-line positioning.
