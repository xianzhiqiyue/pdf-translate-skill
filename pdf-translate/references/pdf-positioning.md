...
- Fonts and exact typography: built-in Helvetica is portable for English/Latin output, but many fonts do not cover Chinese/Japanese/Korean or other non-Latin scripts. Use `recommended_args.font_scripts` and `recommended_args.font_install_hint` from extraction JSON as the font preflight. Install a supporting font if needed, then use `--fontfile auto` or an explicit `.ttf/.otf/.ttc` path. CJK should use a real CJK font such as Noto Sans CJK or Source Han Sans. Arabic/Devanagari/Thai/Hebrew and other complex scripts need visual inspection because glyph coverage and shaping are separate concerns. The script preserves position and color, not exact original font identity.
...
2. Check `recommended_args.font_scripts` before applying. If non-empty, install a matching font and apply with `--fontfile auto` or a known font file.
3. Add `--skip-regex` for page numbers, units-only strings, formulas, citations, or labels that should remain unchanged.
4. Add/adjust glossary entries for professional terms.
5. Apply translations and visually inspect pages at 100% and 200% zoom, especially non-Latin glyphs and complex scripts.
6. Re-run only problematic pages with `--pages` to reduce cost and iteration time.
